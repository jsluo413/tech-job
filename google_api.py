...
def search_jobs(query, site, num_results=10, day=1):
    ...
    if api_key is None or cx is None:
        raise Exception(
            'Failed to get Google API key or CX ID from environment variable')
    ...
    for start in range(0, num_results, 10):
        ...
        if res.status_code == 200:
            ...
            logging.error(
                "something is wrong with google scraping on career site")
            break
...
def filter_links(job_listings):
    grad_keywords = ['Engineeing', 'Software', 'Machine Learning',
                     'Engineer', 'Computer Vision', 'QA', 'Research', "AI", ]
    grad_links = []
